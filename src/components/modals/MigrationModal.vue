           ...
           <div v-if="step === 'check'" class="flex-1 space-y-4">
               <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
                   ...
                   <ul class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
                       ...
                       <li class="flex items-start gap-2">
                            <input type="checkbox" class="mt-1 h-4 w-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500" aria-label="已执行 SQL 建表脚本">
                           <div class="flex flex-col gap-1">
                               <span class="font-medium text-orange-600 dark:text-orange-400">重要：已在 D1 Console 中执行 SQL 建表脚本</span>
                                <button type="button" @click="copySchema" class="text-xs flex items-center gap-1 text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 underline underline-offset-2 transition-colors w-fit" aria-label="复制 SQL 脚本内容">
